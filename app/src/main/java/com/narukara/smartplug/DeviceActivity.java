package com.narukara.smartplug;
    ...
    public void ops(View view) {
        switch (view.getId()) {
            ...
            case R.id.setpower:
                ...
                final Activity context = this;
                builder.setTitle("请输入功率上限（W）").setView(editText).setNegativeButton("取消", null);
//                builder.setTitle("请输入警戒距离（cm）").setView(editText).setNegativeButton("取消", null);
                builder.setPositiveButton("确认", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        String pow = editText.getText().toString();
                        if (pow.equals("")) {
                            Toast.makeText(context, "不能为空", Toast.LENGTH_SHORT).show();
                            return;
                        }
                        Connect.write("PW PL RST " + pow);
                    }
                });
                builder.show();
        }
            ...
            if (Connect.isConnected()) {
                ...
                if (s == null) {
                    ...
                    while (!isInterrupted()) {
                        ...
                        if (s.equals("PL exceeded")) {
                            textView.setText("关停:功率过大");
                        } else if (s.equals("OFF")) {
                            textView.setText("设备关停");
                        } else {
                            textView.setText(s);
                        }
                    }
